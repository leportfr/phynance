...
class gOuter:
    def __init__(self, bsizeA, bsizeB, numvectors):
        ...
        __global__ void MatrixMulKernel(float *A, float *B, float *C)
        {
          ...
          for (int i = 0;
              ...
              j += 1) 
                  {
                  C[tx * %(BLOCK_SIZE_A)s * %(BLOCK_SIZE_B)s + i * %(BLOCK_SIZE_B)s + j] = A[tx * %(BLOCK_SIZE_A)s + i] * B[tx * %(BLOCK_SIZE_B)s + j];
                  }
              }
    ...
    def gOuter(self, a_gpu, b_gpu, c_gpu):
        self.matrixmul(driver.In(a_gpu), driver.In(b_gpu), driver.Out(c_gpu),
...
#    b_cpu = np.concatenate([np.random.randn(VECTOR_SIZE_L).astype(np.float32) for i in range(NUMVECTORS)])
    ...
    c_gpu = np.zeros((NUMVECTORS*VECTOR_SIZE_N,VECTOR_SIZE_L), np.float32)#gpuarray.empty((NUMVECTORS*VECTOR_SIZE_N,VECTOR_SIZE_L), np.float32)    
